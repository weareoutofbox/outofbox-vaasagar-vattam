...
class Deck extends Component {
  constructor(props) {
    ...
    this.state = {
      cards: [
        ...
        <Card
          picsum={`https://picsum.photos/800/353`}
          id='three'
          key='three'
        />,
  ...
  componentDidMount() {
    ...
    this.button_prev.style.width = `${(this.new_width / 2) * 0.3}px`;
    this.button_next.style.width = `${(this.new_width / 2) * 0.3}px`;

    this.selection_buttons_container.style.bottom = `${
      this.view_port.getBoundingClientRect().top
    }px`;
    ...
    window.addEventListener("resize", () => {
      ...
      for (let i = 0; i < this.images.children.length; i++) {
        this.selection_buttons_container.children[i].transitionDuration =
          "0.0s";
        this.selection_buttons_container.children[i].style.width = `${
          this.new_width * 0.05
        }px`;
  ...
  handle_boundaries = () => {
    ...
    if (
      ...
      Math.round(this.right_boundary)
    ) {
